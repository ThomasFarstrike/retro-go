...
void rg_boot_fri3d_app() {
    ...
    // Note: instead of using partition labels, the partition IDs could be used:
    // [0]: type=0, subtype=16, size=1048576 (47% used), label=launcher
    // [1]: type=0, subtype=17, size=851968 (97% used), label=retro-core
    // [2]: type=0, subtype=18, size=1048576 (85% used), label=duke3d-go
    // [3]: type=1, subtype=129, size=5001216 (0% used), label=vfs
    ...
    if (!next_update_partition) {
        ...
        // This can only  happen if the device doesn't have OTA support (so there's only ota_0 and no ota_1)
        // but for some bug, the OTA partition that was saved by the main OTA0/1 Fri3d App was OTA1.
        // This would be a serious issue, so better to refuse doing anything instead of silently ignoring it.
        return; 
    }
