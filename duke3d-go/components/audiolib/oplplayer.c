...
#define Z_Malloc(size, tag, ptr) malloc(size)
...
static uint8_t *genmidi_lump = NULL;
...
// Load instrument table from built-in bank:
...
{
    int opl_vol = volume; // synth internally uses 0-127
...
static void I_OPL_ShutdownMusic(void)
{
    if (music_initialized)
    {
        ...
        OPL_Shutdown();
            ...
            if (genmidi_lump) free(genmidi_lump);
