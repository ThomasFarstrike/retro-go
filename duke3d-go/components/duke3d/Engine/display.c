...
static void init_new_res_vars(int32_t davidoption)
{
	...
	printf("init_new_res_vars %"PRId32" %"PRId32"\n",xdim,ydim);
  	...
  	if (screen != NULL)
   	{
		...
		horizlookup = (int32_t*)rg_alloc(j, MEM_SLOW);
		horizlookup2 = (int32_t*)rg_alloc(j, MEM_SLOW);
...
static __inline int handle_keypad_enter_hack(const SDL_Event *event)
{
    ...
    if (event->key.keysym.sym == SDLK_RETURN)
    {
        if (event->key.state == SDL_PRESSED)
        {
            ...
            {
                kp_enter_hack = 1;
                if (SDLK_KP_ENTER >= 0 && SDLK_KP_ENTER < SDLK_LAST)
...
/*	// Install icon
    ...
    if (daxdim > INTERNAL_RES_W || daydim > INTERNAL_RES_H)
    {
		printf("Resolution %"PRId32"x%"PRId32" is too high. Changed to %"PRId32"x%"PRId32"\n", daxdim, daydim, (int32_t)INTERNAL_RES_W,(int32_t)INTERNAL_RES_H);
	    daxdim = INTERNAL_RES_W;
	    daydim = INTERNAL_RES_H;
    }
	...
	if(!validated){
		printf("Resolution %"PRId32"x%"PRId32" unsupported. Changed to %dx%d\n", daxdim, daydim, INTERNAL_RES_W, INTERNAL_RES_H);
	    daxdim = INTERNAL_RES_W;
	    daydim = INTERNAL_RES_H;
    }
...
static __inline void cull_large_vesa_modes(void)
{
    ...
    printf("Setting resolution ceiling to (%"PRId32"x%"PRId32").\n", max_w, max_h);
...
static __inline void output_vesa_modelist(void)
{
    ...
    for (i = 0; i < validmodecnt; i++)
    {
        sprintf(numbuf, " (%"PRId32"x%"PRId32")",(int32_t) validmodexdim[i], (int32_t) validmodeydim[i]);
...
EXT_RAM_BSS_ATTR uint8_t lastPalette[768];
