...
#define MAXYSAVES 2048
#define MAXWALLSB 1024
#else
#define MAXYSAVES 4096
...
EXT_RAM_BSS_ATTR static short *dotp1[MAXYDIM], *dotp2[MAXYDIM];

EXT_RAM_BSS_ATTR static char tempbuf[MAXWALLS];

int32_t ebpbak, espbak;
EXT_RAM_BSS_ATTR int32_t slopalookup[16384];
...
EXT_RAM_BSS_ATTR uint8_t  picsiz[MAXTILES], tilefilenum[MAXTILES];
...
EXT_RAM_BSS_ATTR char  kensmessage[128];

EXT_RAM_BSS_ATTR uint8_t  britable[16][64];
EXT_RAM_BSS_ATTR uint8_t  textfont[1024], smalltextfont[1024];
...
EXT_RAM_BSS_ATTR pvWall_t pvWalls[MAXWALLSB];

// bunchWallsList contains the list of walls in a bunch.
EXT_RAM_BSS_ATTR static short bunchWallsList[MAXWALLSB];

EXT_RAM_BSS_ATTR static short bunchfirst[MAXWALLSB], bunchlast[MAXWALLSB];

EXT_RAM_BSS_ATTR static short smost[MAXYSAVES], smostcnt;
EXT_RAM_BSS_ATTR static short smoststart[MAXWALLSB];
EXT_RAM_BSS_ATTR static uint8_t  smostwalltype[MAXWALLSB];
EXT_RAM_BSS_ATTR static int32_t smostwall[MAXWALLSB];
static int32_t smostwallcnt = -1L;

EXT_RAM_BSS_ATTR static short maskwall[MAXWALLSB], maskwallcnt;
EXT_RAM_BSS_ATTR static int32_t spritesx[MAXSPRITESONSCREEN];
EXT_RAM_BSS_ATTR static int32_t spritesy[MAXSPRITESONSCREEN+1];

EXT_RAM_BSS_ATTR static spritetype *tspriteptr[MAXSPRITESONSCREEN];
...
EXT_RAM_BSS_ATTR static int32_t lastx[MAXYDIM];
uint8_t  paletteloaded = 0;

#define FASTPALGRIDSIZ 8
EXT_RAM_BSS_ATTR static int32_t rdist[129], gdist[129], bdist[129];
EXT_RAM_BSS_ATTR static uint8_t  colhere[((FASTPALGRIDSIZ+2)*(FASTPALGRIDSIZ+2)*(FASTPALGRIDSIZ+2))>>3];
EXT_RAM_BSS_ATTR static uint8_t  colhead[(FASTPALGRIDSIZ+2)*(FASTPALGRIDSIZ+2)*(FASTPALGRIDSIZ+2)];
EXT_RAM_BSS_ATTR static int32_t colnext[256];
...
EXT_RAM_BSS_ATTR static linetype clipit[MAXCLIPNUM];
EXT_RAM_BSS_ATTR static short clipsectorlist[MAXCLIPNUM], clipsectnum;
EXT_RAM_BSS_ATTR static short clipobjectval[MAXCLIPNUM];
...
EXT_RAM_BSS_ATTR static permfifotype permfifo[MAXPERMS];
...
{   // Ken did this
    ...
    return((reciptable[(res>>12)&2047]>>(((res-0x3f800000)>>23)&31))^(res>>31));
}
              ...
              int32_t *daposz, short *daang, short *dacursectnum)
{
    ...
    // printf("loadboard: zeroing arrays\n");
...
EXT_RAM_BSS_ATTR extern uint8_t lastPalette[768];
...
IRAM_ATTR void nextpage(void)
{
    ...
    // printf("nextpage: calling _nextpage\n"); fflush(stdout);
...
void clearview(int32_t dacol)
{
    ...
    // printf("clearview: entry, dacol=%"PRId32", qsetmode=%"PRId32"\n", dacol, qsetmode);
    ...
    // printf("clearview: dx=%"PRId32", frameplace=%p, ylookup[windowy1]=%"PRId32", windowx1=%"PRId32", p=%p\n", dx, frameplace, ylookup[windowy1], windowx1, p);
