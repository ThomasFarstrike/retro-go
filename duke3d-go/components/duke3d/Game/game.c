...
{
    return engine_exit_requested;
}

void rg_system_request_exit(void)
{
    engine_exit_requested = true;
}
...
int gametext(int x,int y,const char  *t,uint8_t  s,short dabits)
{
    short ac,newx;
    const char* oldt;
    uint8_t  centre;
...
const char  *grpVersion2char_from_crc(unsigned int crc32_grp_to_identify)
{
	const char  *id;
	int i=0;
...
const char  *grpVersion2char(uint8_t  grp_to_identify)
{
    ...
    while ((packbufleng = getpacket(&other,packbuf)) > 0)
    {
        ...
        switch(packbuf[0])
        {
            ...
            case 17:
                ...
                if (j > packbufleng)
				{
					printf("INVALID GAME PACKET!!! (%ld too many bytes) (j= %ld, packbuflen= %ld, type: %d)\n",(long)(j-packbufleng), (long)j, (long)packbufleng, packbuf[0]);
				}
			...
			case 132: // get map CRC of opponents (to debug out of synch)
				ud.mapCRC_array[other] = (uint16_t)packbuf[1] + (uint16_t)(packbuf[2]<<8);
				break;
...
void caches(void)
{
    ...
    char text[512];
     ...
     for(i=0;i<cacnum;i++)
          if ((*cac[i].lock) >= 200)
          {
                sprintf(text,"Locked- %d: Leng:%ld, Lock:%d",i,(long)cac[i].leng,*cac[i].lock);
...
void dispVersion(void)
{
	...
	for(i=connecthead;i>=0;i=connectpoint2[i])
	{
		...
		if(ps[i].fakeplayer)
			...
			sprintf(text,"MAP CRC: %lX", (unsigned long)ud.mapCRC_array[i]);
...
void checksync(void)
{
      ...
      if (syncstat)
      {
			...
			for(i=connecthead;i>=0;i=connectpoint2[i])
			{
				if (ud.mapCRC_array[connecthead]!=ud.mapCRC_array[i])
				{
...
void displayfragbar(void)
{
    ...
    for(i=connecthead;i>=0;i=connectpoint2[i])
    {
        ...
        sprintf(text,"%ld",(long)(ps[i].frag-ps[i].fraggedself));
...
void coolgaugetext(short snum)
{
    ...
    if (ss == 4)   //DRAW MINI STATUS BAR:
    {
		...
		if(ud.extended_screen_size>0)
		{
			...
			sprintf(text,"%ld", (long)ps[screenpeek].ammo_amount[ps[screenpeek].curr_weapon]);
			minitext(offx+26,offy+21,text,COLOR_ON,2+8+16); //minitext: 2 red light, 23 yellow
			sprintf(text,"%ld", (long)ps[screenpeek].last_extra);
...
void tics(short offx, short offy, short color)
{
	...
	if( mapname[0] != 0 && ud.m_level_number == 7 && ud.m_volume_number == 0 )
		...
		sprintf(text, "e%ldl%ld", (long)ud.volume_number+1, (long)ud.level_number+1);
...
void coords(short snum)
{
    ...
    sprintf(text,"SEED= %ld",(long)randomseed);
...
// CTW END - MODIFICATION
    ...
    extern volatile bool reboot_ready_flag;
    reboot_ready_flag = true;
    
    // We must not return to the calling code or main loop because 
    // ShutDown() has already cleaned up memory and hardware.
    vTaskDelete(NULL);
}
...
#define FOFTILEX 32
#define FOFTILEY 32
EXT_RAM_BSS_ATTR int32_t tempsectorz[MAXSECTORS];
EXT_RAM_BSS_ATTR int32_t tempsectorpicnum[MAXSECTORS];
...
short EGS(short whatsect,int32_t s_x,int32_t s_y,int32_t s_z,short s_pn,int8_t s_s,int8_t s_xr,int8_t s_yr,short s_a,short s_ve,int32_t s_zv,short s_ow,int8_t s_ss)
{
    ...
    if( i < 0 )
    {
        RG_LOGW("EGS: Too many sprites! Failed to spawn picnum:%d\n", s_pn);
        return -1;
    }
...
{
    short i, s, startwall, endwall, sect, clostest = -1;
    int32_t x, y, d;
    spritetype *sp;
    char text[512];
                ...
                switch(sp->lotag)
                {
                    ...
                    case 30://No rotational subways
                        ...
                        if(sp->lotag == 0)
                        {
                            ...
                            if(j == MAXSPRITES)
                            {
                                sprintf(text,"Found lonely Sector Effector (lotag 0) at (%ld,%ld)\n",(long)sp->x,(long)sp->y);
                                gameexit(text);
                            }
                            sp->owner = j;
                        }
                        ...
                        for(s=startwall;s<endwall;s++)
                        {
                            ...
                            if(tempwallptr > 2047)
                            {
                                sprintf(text,"Too many moving sectors at (%ld,%ld).\n",(long)wall[s].x,(long)wall[s].y);
                                gameexit(text);
                            }
                        }
...
void printstr(short x, short y, uint8_t  string[], uint8_t  attribute)
{
        uint8_t  character;
        short i, pos;
...
void getnames(void)
{
    ...
    for(l=0; l<=9 && duke_myname[l];l++)
    {
        ud.user_name[myconnectindex][l] = toupper(duke_myname[l]);
        buf[l+2] = toupper(duke_myname[l]);
    }
...
//        getpackets();
		...
		if(l)
		{
			...
			for(i=connecthead;i>=0;i=connectpoint2[i])
			{
				...
				{
					if(j)
						                                                printf("                    GRP (Add-on) : %s CRC=%lX\n",
						                                                grpVersion2char_from_crc(ud.groupefil_crc32[i][j]),
						                                                (unsigned long)ud.groupefil_crc32[i][j]);
						
					else
						                                                printf("Player [%-10s] GRP (base)   : %s CRC=%lX\n", ud.user_name[i],
						                                                grpVersion2char_from_crc(ud.groupefil_crc32[i][j]),
						                                                (unsigned long)ud.groupefil_crc32[i][j]);
...
int main(int argc,char  **argv)
{
	...
	{
		                printf( "The content of your original BASE *.GRP is corrupted. CRC=%lX\n"
		                        "You may run in troubles. Official GRP are:\n\n", (unsigned long)groupefil_crc32[0]);
		

		for(i=0; i<MAX_KNOWN_GRP; i++)
			                        printf("%s -> CRC32=%lX  Size=%ld bytes\n", crc32lookup[i].name, (unsigned long)crc32lookup[i].crc32, (long)crc32lookup[i].size);
   ...
   puts("Loading palette/lookups.");
    ...
    while ( !(ps[myconnectindex].gm&MODE_END) && !rg_system_should_exit() )
    {
    	sampletimer();
...
uint8_t  opendemoread(uint8_t  which_demo) // 0 = mine
{
	...
	if (PLUTOPAK)
	{
		...
		{
			                        printf("%s is a demo version %ld. We want v. %ld, %ld, %ld, or %ld (1.5 Atomic versions)\n",
			                                        fname, (long) ver, (long)BYTEVERSION_116, (long)BYTEVERSION_117, (long)BYTEVERSION_118, (long)BYTEVERSION);
	...
	else // 1.3/1.3d style
	{
		...
		{
			                        printf("%s is a demo version %ld. We want v. %ld, %ld, %ld or %ld (1.3/1.3d versions)\n",
			                                        fname, (long) ver, (long)BYTEVERSION_27, (long)BYTEVERSION_28, (long)BYTEVERSION_29, (long)BYTEVERSION);
	...
	if(ver==BYTEVERSION)
	{
		...
		for(i=0; i<MAXGROUPFILES; i++)
			if(groupefil_crc32_from_demo[i]!=groupefil_crc32[i])
			{
				for(j=0; j<=i; j++)
				{
					                                        printf("You have GRP #%d:  %s (CRC32=%lX)\n"
					                                                   "this demo expects %s (CRC32=%lX)\n",
					                                                j, grpVersion2char_from_crc(groupefil_crc32[j]),(unsigned long)groupefil_crc32[j],
					                                                grpVersion2char_from_crc(groupefil_crc32_from_demo[j]),(unsigned long)groupefil_crc32_from_demo[j]);
									}
				kclose(recfilep);
				return 0;
			}
...
void record(void)
{
    ...
    for(i=connecthead;i>=0;i=connectpoint2[i])
         {
         copybufbyte(&dukeSyncArray[i],&recsync[ud.reccnt],sizeof(input));
			   ud.reccnt++;
                 totalreccnt++;
...
uint8_t  domovethings(void)
{
    ...
    for(i=connecthead;i>=0;i=connectpoint2[i])
        if( dukeSyncArray[i].bits&(1<<17) )
    {
        multiflag = 2;
        multiwhat = (dukeSyncArray[i].bits>>18)&1;
        multipos = (uint32_t) (dukeSyncArray[i].bits>>19)&15;
        multiwho = i;
...
//            waitforeverybody();
    ...
    for(i=connecthead;i>=0;i=connectpoint2[i])
        copybufbyte(&inputfifo[movefifoplc&(MOVEFIFOSIZ-1)][i],&dukeSyncArray[i],sizeof(input));
    movefifoplc++;
    ...
    for(i=connecthead;i>=0;i=connectpoint2[i])
     {
          if ((dukeSyncArray[i].bits&(1<<26)) == 0) { j = i; continue; }
            ...
            if(ud.lockout == 0)
            {
                ...
                while( 1 )
                {
                    ...
                    if( totalclock > 390 && totalclock < 780 )
                        for(t=0;t<=25;t+=5) if( bossmove[t+2] && (totalclock%390) > bossmove[t] && (totalclock%390) <= bossmove[t+1] )
                    {
    ...
    if(playerswhenstarted > 1 && ud.coop != 1 )
    {
        ...
        for(i=0;i<playerswhenstarted;i++)
        {
            ...
            for(y=0;y<playerswhenstarted;y++)
            {
                if(i == y)
                {
                    sprintf(text,"%-4ld",(long)ps[y].fraggedself);
                    minitext(92+(y*23),90+t,text,2,2+8+16+128);
                    xfragtotal -= ps[y].fraggedself;
                }
                else
                {
                    sprintf(text,"%-4ld",(long)frags[i][y]);
                ...
                if(myconnectindex == connecthead)
                {
                    sprintf(text,"stats %ld killed %ld %ld\n",(long)i+1,(long)y+1,(long)frags[i][y]);
                    sendscore(text);
                }
            }

            sprintf(text,"%-4ld",(long)xfragtotal);
        ...
        for(y=0;y<playerswhenstarted;y++)
        {
            ...
            sprintf(text,"%-4ld",(long)yfragtotal);
        ...
        if(ps[myconnectindex].gm&MODE_EOL)
        {
            ...
            if( totalclock > (60*3) )
            {
                ...
                if( totalclock > (60*4) )
                {
                    ...
                    sprintf(text,"%02ld:%02ld",
                        (long)(ps[myconnectindex].player_par/(26*60))%60,
                        (long)(ps[myconnectindex].player_par/26)%60);
                    gametext((320>>2)+71,60+9,text,0,2+8+16);

                    sprintf(text,"%02ld:%02ld",
                        (long)(partime[ud.volume_number*11+ud.last_level-1]/(26*60))%60,
                        (long)(partime[ud.volume_number*11+ud.last_level-1]/26)%60);
                    gametext((320>>2)+71,69+9,text,0,2+8+16);

                    sprintf(text,"%02ld:%02ld",
                        (long)(designertime[ud.volume_number*11+ud.last_level-1]/(26*60))%60,
                        (long)(designertime[ud.volume_number*11+ud.last_level-1]/26)%60);
...
void takescreenshot(void)
{
	...
	if(SafeFileExists(szFilename) == 0) // returns 1 if exists, 0 if not
	{
		...
		snprintf(fta_quotes[103], sizeof(fta_quotes[103]), "SCREEN SAVED");
		sound(EXITMENUSOUND);
	}
	else
		snprintf(fta_quotes[103], sizeof(fta_quotes[103]), "FILE ALREADY EXISTS!");
