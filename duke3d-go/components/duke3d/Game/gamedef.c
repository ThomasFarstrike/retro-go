...
void getlabel(void)
{
    ...
    while( isalnum((unsigned char)*textptr) == 0 )
    {
...
void transnum(void)
{
    ...
    if( isdigit((unsigned char)*textptr) == 0 && *textptr != '-')
    {
...
void loadefs(char  *filenam, char  *mptr, int readfromGRP)
{
    ...
    clearbuf(actorscrptr,MAXTILES,0L);
    ...
    *script = (int32_t) scriptptr;
    ...
    {
        total_lines += line_number;
        printf("Code Size:%ld bytes(%ld labels).\n",(long)((scriptptr-script)<<2)-4,(long)labelcnt);
        ...
        case 51:
            {
                ...
                if( (l&8) && ps[g_p].on_ground && (dukeSyncArray[g_p].bits&2) )
                       j = 1;
                    ...
                    ps[g_p].poszv > 2048 )
                       ...
                       j = 1;
                else if( (l&2) && s >= 8 && !(dukeSyncArray[g_p].bits&(1<<5)) )
                       j = 1;
                else if( (l&4) && s >= 8 && dukeSyncArray[g_p].bits&(1<<5) )
                       j = 1;
                else if( (l&64) && ps[g_p].posz < (g_sp->z-(48<<8)) )
                       j = 1;
                else if( (l&128) && s <= -8 && !(dukeSyncArray[g_p].bits&(1<<5)) )
                       j = 1;
                else if( (l&256) && s <= -8 && (dukeSyncArray[g_p].bits&(1<<5)) )
                       j = 1;
...
//            else
        ...
        case 62:
            ...
            break;
        case 63:
            parseifelse( dukeSyncArray[g_p].bits&(1<<29));
            break;
        case 64:
