...
EXT_RAM_BSS_ATTR static char menuname[256][17];
...
int loadpheader(uint8_t  spot,int32 *vn,int32 *ln,int32 *psk,int32 *nump)
{
    ...
    int32_t bv;
     ...
     const char *save_path = get_save_path(fn);
     if ((fil = TCkopen4load(save_path,0)) == -1) return(-1);
...
static EXT_RAM_BSS_ATTR char scriptptrs[MAXSCRIPTSIZE];
...
int loadplayer(int8_t spot)
{
     ...
     const char *save_path = get_save_path(fnptr);
     if ((fil = TCkopen4load(save_path,0)) == -1) {
         RG_LOGE("loadplayer: FAILED to open '%s'\n", save_path);
         return(-1);
     }
...
int saveplayer(int8_t spot)
{
    ...
    if (!rg_storage_exists(RG_BASE_PATH_SAVES "/duke3d")) {
        RG_LOGW("saveplayer: Save directory missing, attempting to create it\n");
        rg_storage_mkdir(RG_BASE_PATH_SAVES);
        rg_storage_mkdir(RG_BASE_PATH_SAVES "/duke3d");
    }

    strcpy(fullpathsavefilename, save_path);

     if ((fil = fopen(fullpathsavefilename,"wb")) == 0) {
         RG_LOGE("saveplayer: FAILED to open '%s' for writing!\n", fullpathsavefilename);
         return(-1);
     }
    ...
    for(i=0;i<MAXSPRITES;i++)
    {
     ...
     dfwrite(&parallaxyscale,sizeof(parallaxyscale),1,fil);
         ...
         fclose(fil);
         rg_storage_commit();
         // RG_LOGI("saveplayer: successfully saved to '%s'\n", fullpathsavefilename);
...
void menus(void)
{
    ...
    switch(current_menu)
    {
        ...
        case 1009:
            ...
            sprintf((char*)tempbuf,"PLAYERS: %-2"PRId32"                      ",numplr);
            gametext(160,158,(char*)tempbuf,0,2+8+16);

            sprintf((char*)tempbuf,"EPISODE: %-2"PRId32" / LEVEL: %-2"PRId32" / SKILL: %-2"PRId32, (int32_t)(1+volnum), (int32_t)(1+levnum), (int32_t)plrskl);
        ...
        case 2009:
            ...
            sprintf(text,"EPISODE: %-2"PRId32" / LEVEL: %-2"PRId32" / SKILL: %-2"PRId32, (int32_t)(1+ud.volume_number), (int32_t)(1+ud.level_number), (int32_t)ud.player_skill);
...
else
{
        ...
        case 110:
            ...
            if(x >= 0)
            {
                ...
                // RG_LOGI("Starting new game: vol=%ld, lev=%ld, skill=%ld", (long)ud.m_volume_number, (long)ud.m_level_number, (long)ud.m_player_skill);
				
                newgame(ud.m_volume_number,ud.m_level_number,ud.m_player_skill);
                // RG_LOGI("Calling enterlevel...");
                enterlevel(MODE_GAME);
                // RG_LOGI("Back from enterlevel.");
            }
            else if(x == -1)
            {
                cmenu(100);
        ...
        case 706: // Video setup
			...
			sprintf(text, "%"PRId32" x %"PRId32, validmodexdim[current_resolution],validmodeydim[current_resolution]);
        ...
        case 300:
            ...
            if(current_menu >= 360 && current_menu <= 369 )
            {
                ...
                // Safety delay: ignore button clicks for 250ms after entering this menu
                static int32_t enter_time = 0;
                static int last_cm = -1;
                if (current_menu != last_cm) {
                    enter_time = totalclock;
                    last_cm = current_menu;
                }
                if (totalclock > enter_time + 30 && (LMB || KB_KeyPressed(sc_Enter) || KB_KeyPressed(sc_Space))) x = 1;
            ...
            //        readsavenames();
                ...
                if( x == 1 )
                {
                    ...
                    {
                        // No name entered, use a default one since we might not have a keyboard
                        strcpy(&ud.savegame[current_menu-360][0], "Save");
                    }
		...
		case 502: // from menu 50
        {
            ...
            // Safety delay: ignore input for 250ms after entering this menu
            static int32_t enter_time = 0;
            static int last_cm = -1;
            if (current_menu != last_cm) {
                enter_time = totalclock;
            ...
            if (totalclock >= enter_time + 30) 
            {
                ...
                x = probe(186,124,0,0);
                if(x == -1 || KB_KeyPressed(sc_N) || RMB || KB_KeyPressed(sc_Escape))
                {
                    KB_ClearKeyDown(sc_N);
                    KB_ClearKeyDown(sc_Escape);
                    quittimer = 0;
...
if (VOLUMEONE)
{            sprintf(text,"EPISODE %"PRId32, (int32_t)(ud.m_volume_number+1));
...
#ifndef ONELEVELDEMO
            sprintf(text,"LEVEL %"PRId32, (int32_t)(ud.m_level_number+1));
...
void palto(uint8_t  r,uint8_t  g,uint8_t  b,int32_t e)
{
    ...
    if (ps[myconnectindex].palette == NULL) {
        printf("palto: ps[myconnectindex].palette is NULL!\n");
        return;
    }
...
void playanm(char  *fn,uint8_t  t)
{
    ...
    if(anim == 0 || lastanimhack != (MAXTILES-3-t)) {
        ...
        if (anim == NULL) {
            printf("playanm: allocache FAILED for %s, length=%"PRIi32"\n", fn, length);
            RG_PANIC("Animation allocation failed!");
        }
    }
    ...
    printf("playanm: %s loading %"PRIi32" bytes\n", fn, length);
    kread(handle,animbuf,length);
    kclose(handle);

    ANIM_LoadAnim (animbuf);
    numframes = ANIM_NumFrames();
    printf("playanm: %s %"PRIi32" frames\n", fn, numframes);
