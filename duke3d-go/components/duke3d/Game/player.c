...
void displayloogie(short snum)
{
    ...
    for(i=0;i<ps[snum].numloogs;i++)
    {
        ...
        x = (-dukeSyncArray[snum].avel)+(sintable[((ps[snum].loogcnt+i)<<6)&2047]>>10);
...
uint8_t  animatefist(short gs,short snum)
{
    ...
    rotatesprite(
        (-fisti+222+(dukeSyncArray[snum].avel>>4))<<16,
...
uint8_t  animateknee(short gs,short snum)
{
    ...
    myospal(105+(dukeSyncArray[snum].avel>>4)-(ps[snum].look_ang>>1)+(knee_y[ps[snum].knee_incs]>>2),looking_arc+280-((ps[snum].horiz-ps[snum].horizoff)>>4),KNEE,gs,4,pal);
...
uint8_t  animateknuckles(short gs,short snum)
{
    ...
    myospal(160+(dukeSyncArray[snum].avel>>4)-(ps[snum].look_ang>>1),looking_arc+180-((ps[snum].horiz-ps[snum].horizoff)>>4),CRACKKNUCKLES+knuckle_frames[ps[snum].knuckle_incs>>1],gs,4,pal);
...
/*    if(ps[snum].access_spritenum >= 0)
  ...
  */
    myospal(170+(dukeSyncArray[snum].avel>>4)-(ps[snum].look_ang>>1),
...
//        p = wall[ps[snum].access_wallnum].pal;
    ...
    if((ps[snum].access_incs-3) > 0 && (ps[snum].access_incs-3)>>3)
        myospal(170+(dukeSyncArray[snum].avel>>4)-(ps[snum].look_ang>>1)+(access_y[ps[snum].access_incs]>>2),looking_arc+266-((ps[snum].horiz-ps[snum].horizoff)>>4),HANDHOLDINGLASER+(ps[snum].access_incs>>3),gs,0,p);
    else
        myospal(170+(dukeSyncArray[snum].avel>>4)-(ps[snum].look_ang>>1)+(access_y[ps[snum].access_incs]>>2),looking_arc+266-((ps[snum].horiz-ps[snum].horizoff)>>4),HANDHOLDINGACCESS,gs,4,p);
...
//    p->weapon_ang = -(j/5);
    ...
    if(p->knuckle_incs)
    {
        ...
        else if( p->knuckle_incs == 22 || (dukeSyncArray[snum].bits&(1<<2)))
            p->knuckle_incs=0;
...
//            p->bobcounter += 32;
    ...
    j = ksgn(dukeSyncArray[snum].avel);
    /*
    ...
    */
    ...
    else if ( dukeSyncArray[snum].avel )          //p->ang += syncangvel * constant
    {                         //ENGINE calculates angvel for you
        int32_t tempang;

        tempang = dukeSyncArray[snum].avel<<1;
    ...
    if ( p->posxv || p->posyv || dukeSyncArray[snum].fvel || dukeSyncArray[snum].svel )
    {
        p->crack_time = 777;
        ...
        p->posxv += ((dukeSyncArray[snum].fvel*doubvel)<<6);
        p->posyv += ((dukeSyncArray[snum].svel*doubvel)<<6);
    ...
    HORIZONLY:
        ...
        if(p->aim_mode)
            p->horiz += dukeSyncArray[snum].horz>>1;
        else
        {
