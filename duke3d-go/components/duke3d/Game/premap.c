...
{
    short i, j;
    printf("precachenecessarysounds: %ld\n", (long)SoundToggle);
...
void newgame(uint8_t  vn,uint8_t  ln,uint8_t  sk)
{
    ...
    {
        int32_t timeout = 1000000; // Safety timeout
        while(Sound[globalskillsound].lock>=200 && timeout--)
        {
            // Just wait, but don't hang forever
            if ((timeout % 1000) == 0) _handle_events();
        }
    }
    globalskillsound = -1;
...
void clearfifo(void)
{
    ...
    clearbufbyte(&dukeSyncArray[0],sizeof(sync),0L);
...
if (!VOLUMEONE)
{
    // RG_LOGI("enterlevel: NOT VOLUMEONE");
    
    if( boardfilename[0] != 0 && ud.m_level_number == 7 && ud.m_volume_number == 0 )
    {
        // RG_LOGI("enterlevel: User Map path...");
        ...
        // RG_LOGI("enterlevel: loading board '%s'...", fulllevelfilename);
        if ( loadboard( fulllevelfilename,&ps[0].posx, &ps[0].posy, &ps[0].posz, &ps[0].ang,&ps[0].cursectnum ) == -1 )
        {
            snprintf(text,sizeof(text),"User Map %s not found!\n",fulllevelfilename);
            gameexit(text);
        }
    }
    else
    {
        // RG_LOGI("enterlevel: Internal Map path...");
	...
	// RG_LOGI("enterlevel: loading board '%s'...", levname);
    ...
    // RG_LOGI("enterlevel: prelevel...");
    prelevel(g);

    // RG_LOGI("enterlevel: alignment...");
    allignwarpelevators();
    // RG_LOGI("enterlevel: resetting pspritevars...");
    resetpspritevars(g);
...
//    if(ud.level_number == 0 && ud.recstat != 2) FTA(40,&ps[myconnectindex]);
    ...
    for(i=connecthead;i>=0;i=connectpoint2[i])
	 ...
	 ud.mapCRC_array[myconnectindex] = mapCRC;
